...
public class GenLedgerSummaryActionBean extends BaseBean {
	...
	public void searchReportList() {
		try {
			...
			List<GenLedgerSummaryDTO> beforeStartDateList = genLedgerSummaryDTOList.stream()
					.filter(dto -> dto.getSettlementDate()
							.before(resetStartDate))
			...
			List<GenLedgerSummaryDTO> afterStartDateList = genLedgerSummaryDTOList.stream()
					.filter(dto -> dto.getSettlementDate().equals(resetStartDate) || dto.getSettlementDate()
							.after(resetStartDate))
					.collect(Collectors.toList());
			
			if (afterStartDateList.size() > 0) {
				/* FOR TEST */
//				afterStartDateList = genLedgerSummaryDTOList.stream().collect(Collectors.toList());
			...
			afterStartDateList = afterStartDateList.stream()
					.filter(dto -> set.add(dto.getAccountCode()))
					.collect(Collectors.toList());
