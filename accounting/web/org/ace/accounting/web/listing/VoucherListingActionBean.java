...
public class VoucherListingActionBean extends BaseBean {
	...
	public void search() {
		if (validate()) {
			List<VLdto> dtoList = listingReportService.findVoucherListingReport(type, startDate, endDate, branch, currency,allVoucherType?null: Boolean.valueOf(isOnlyVoucher),reverse);
			if (dtoList.size() == 0) {
